    ...
    try:
        while True:
            ...
            for chunk_name, total_bytes in list(job_manager.chunk_totals.items()):
                # tqdm.write(f"Main total_bytes: {total_bytes}")
                tqdm_manager.create_bar(
                    bar_type=BAR_TYPE_CHUNK,
                    bar_id=chunk_name,
                    total=total_bytes,
                    metadata={}, 
                    unit='B', 
            ...
            for chunk_name in list(job_manager.chunk_progress.keys()):
                ...
                tqdm_manager.progress(chunk_name, chunk_progress_value)
                ...
                if chunk_progress_value >= job_manager.chunk_totals.get(chunk_name, 0) and chunk_name not in completed_once:
                    ...
                    tqdm_manager.finish_bar(chunk_name)
