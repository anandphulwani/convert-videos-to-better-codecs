    ...
    with _progress_lock:
        ...
        with tqdm(total=total_size, unit='B', unit_scale=True, desc=desc, leave=False) as pbar:
            if os.path.isfile(src):
                ...
                # log(f"05.01. Copying src: {src}")
                for dirpath, _, files in os.walk(src):
                    rel_path = os.path.relpath(dirpath, src)
                    target_dir = os.path.join(dst, rel_path)
                    os.makedirs(target_dir, exist_ok=True)
                    # log(f"05.02. target_dir: {target_dir}")
                    for file in files:
                        src_file = os.path.join(dirpath, file)
                        dst_file = os.path.join(target_dir, file)
                        # log(f"05.03. Copying from: {src_file} to {dst_file}")
                        with open(src_file, 'rb') as fsrc, open(dst_file, 'wb') as fdst:
                            while True:
                                ...
                                fdst.write(buf)
                                pbar.update(len(buf))
                        # log(f"05.04. Copying Stats from: {src_file} to {dst_file}")
                        shutil.copystat(src_file, dst_file)
                # log(f"05.05. Copying Stats from: {src} to {dst}")
...
def move_with_progress(src, dst, remove_empty_source=True, move_contents_not_dir_itself=False, desc="Moving"):
    with _progress_lock:
        if move_contents_not_dir_itself and os.path.isdir(src):
            # log(f"01. In move_contents_not_dir_itself")
            os.makedirs(dst, exist_ok=True)
            ...
            # log(f"02. Not in `move_contents_not_dir_itself`.")
            copy_with_progress(src, dst, desc)
            # log(f"03. Removing the source (file/dir): {src}")
            if os.path.isfile(src):
                os.remove(src)
            else:
                shutil.rmtree(src)
            if remove_empty_source:
                # log(f"04. `remove_empty_source` is set to true.")
